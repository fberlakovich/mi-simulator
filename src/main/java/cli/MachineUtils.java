...
public class MachineUtils {
     ...
     *
     * @param machine     the machine context to load the program into
     * @param programText the source code of the MI program
     * @return true if assembly and loading succeeded, false otherwise
     */
    public static boolean assembleAndLoad(MachineContext machine, String programText) {
        machine.reset();
        Scanner scanner = new Scanner(false);
        scanner.init(programText);
        // Parser requires Machine (not MachineContext) for internal operations
     ...
     *
     * @param machine the machine context to load state into
     ...
     */
    public static void loadState(MachineContext machine, String state, boolean useHex) {
        ...
        for (String line : lines) {
            ...
            try {
                if (key.matches("R\\d+")) {
                    loadRegister(machine, key, value, radix, lineNum);
                } else if (key.equals("C")) {
                    machine.getFlags().setCarry(parseFlag(value, radix));
                } else if (key.equals("V")) {
                    machine.getFlags().setOverflow(parseFlag(value, radix));
                } else if (key.equals("Z")) {
                    machine.getFlags().setZero(parseFlag(value, radix));
                } else if (key.equals("N")) {
                    machine.getFlags().setNegative(parseFlag(value, radix));
                } else {
                    loadMemory(machine, key, value, radix, lineNum);
                }
    ...
    private static void loadRegister(MachineContext machine, String key, String value, int radix, int lineNum) {
        ...
        machine.getRegisters().getRegister(regNum).setContent(
                NumberConversion.intToByte(regValue, 4));
    }

    private static void loadMemory(MachineContext machine, String key, String value, int radix, int lineNum) {
        ...
        machine.getMemory().setContent(address, new MyByte[]{new MyByte(memValue)});
    }
