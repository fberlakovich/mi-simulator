 ...
 *
 */
public class Jbssi extends Command {
     ...
     */
    public Jbssi(Machine machine, int line, int adress, Operand op1, Operand op2, Operand op3, int beg,
                 int end) {
    ...
    /**
     * Decodes a JBSSI instruction from memory.
     * MI Manual page 93: JBSSI with 3 operands (jump if bit set, set and invert).
     * Opcode: 0xFB
     */
    public static Command decode(Machine machine, int pc, Opcode opcode) {
        ...
        return new Jbssi(machine, 0, pc, op1, op2, op3, 0, 0);
    }
    @Override
    ...
    public synchronized void run() {
        // JBSSI bitPos, register, label
        // LSB-0 bit numbering throughout
        // Semantics: JBSSI N tests bit N, if set then jumps
        // Always sets the 2-bit pair containing N: bits (N & ~1) and (N | 1)
        // This means: N=0,1 both set bits 0,1; N=2,3 both set bits 2,3; etc.
        ...
        // Test if bit at bitPos (LSB-0) is set
        boolean bitIsSet = ((value >>> bitPos) & 1) != 0;

        // Always set the 2-bit pair containing bitPos
        int evenBit = bitPos & ~1;  // Clear LSB to get even bit
        int oddBit = bitPos | 1;     // Set LSB to get odd bit
        value |= (1 << evenBit);
        value |= (1 << oddBit);
        op2.setContent(NumberConversion.intToByte(value, 4), 4);

        // Jump if bit bitPos was set before we modified
        if (bitIsSet) {
            int target = 0;
    ...
    public String toString() {
        ...
        return "JBSSI " + op1.toString() + ", " + op2.toString() + ", " + op3.toString();
    }
}
