...
/**
 * Repräsentation eines Mult-Befehls
 *
...
public class Mult extends Command {
    ...
    /**
     * Konstruktor für einen 2-Adress MULT-Befehl
     *
     ...
     */
    public Mult(Machine machine, int line, int adress, int length, Operand op1, Operand op2, int beg, int end,
                boolean floating) {
    ...
    /**
     * Konstruktor für einen 3-Adress MULT-Befehl
     *
     * @param machine  the machine this command operates on
     * @param line     Zeile im Quelltext
     * @param adress   Adresse des Befehls
     ...
     * @param floating true, falls es ein Gleitpunktzahlbefehl ist
     */
    public Mult(Machine machine, int line, int adress, int length, Operand op1, Operand op2, Operand op3,
    ...
    /**
     * Decodes a MULT instruction from memory.
     * MI Manual page 92: MULT B/H/W/F/D with 2 or 3 operands.
     * Opcodes: 0xD3-0xDC
     */
    public static Command decode(Machine machine, int pc, Opcode opcode) {
        ...
        if (opcode.operandCount == 3) {
            ...
            return new Mult(machine, 0, pc, opcode.length, op1, op2, op3, 0, 0, opcode.floating);
        }
        return new Mult(machine, 0, pc, opcode.length, op1, op2, 0, 0, opcode.floating);
    }
    ...
    public byte[] encode() {
        ...
        switch (length) {
            case 1:
                opcode = three ? new MyByte("D8") : new MyByte("D3");
                break;
            case 2:
                opcode = three ? new MyByte("D9") : new MyByte("D4");
                break;
            case 4:
                opcode = three ?
                        (floating ? new MyByte("DB") : new MyByte("DA")) :
                        (floating ? new MyByte("D6") : new MyByte("D5"));
                break;
            case 8:
