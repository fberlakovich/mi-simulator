...
public class Sh extends Command {
     ...
     */
    public Sh(Machine machine, int line, int adress, Operand op1, Operand op2, Operand op3, int beg, int end) {
    ...
    /**
     * Decodes a SH instruction from memory.
     * MI Manual page 92: SH with 3 operands, Word type.
     * Opcode: 0xE7
     */
    ...
    public synchronized void run() {
        ...
        int val = NumberConversion.myBytetoIntWithSign(ope2);
        int res;
        ...
        if (ope1 < 0) {
            ...
            // Left shift
            res = val << ope1;
            // Check overflow for left shift
            // Overflow if result does not match logic shift on larger container
            long valLong = val;
            long resLong = valLong << ope1;
            if (resLong != res) {
                overflow = true;
            }
        }
