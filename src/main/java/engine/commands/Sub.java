...
public class Sub extends Command {
    ...
    /**
     * Konstruktor für einen 2-Adress SUB-Befehl
     *
     ...
     */
    public Sub(Machine machine, int line, int adress, int length, Operand op1, Operand op2, int beg, int end,
               boolean floating) {
    ...
    /**
     * Konstruktor für einen 3-Adress SUB-Befehl
     *
     * @param machine  the machine this command operates on
     * @param line     Zeile im Quelltext
     * @param adress   Adresse des Befehls
     ...
     * @param floating true, falls es ein Gleitpunktzahlbefehl ist
     */
    public Sub(Machine machine, int line, int adress, int length, Operand op1, Operand op2, Operand op3, int beg,
    ...
    /**
     * Decodes a SUB instruction from memory.
     * MI Manual page 91: SUB B/H/W/F/D with 2 or 3 operands.
     * Opcodes: 0xC9-0xD2
     */
    public static Command decode(Machine machine, int pc, Opcode opcode) {
        ...
        if (opcode.operandCount == 3) {
            ...
            return new Sub(machine, 0, pc, opcode.length, op1, op2, op3, 0, 0, opcode.floating);
        }
        return new Sub(machine, 0, pc, opcode.length, op1, op2, 0, 0, opcode.floating);
    }
    ...
    public byte[] encode() {
        ...
        switch (length) {
            case 1:
                opcode = three ? new MyByte("CE") : new MyByte("C9");
                break;
            case 2:
                opcode = three ? new MyByte("CF") : new MyByte("CA");
                break;
            case 4:
                opcode = three ?
                        (floating ? new MyByte("D1") : new MyByte("D0")) :
                        (floating ? new MyByte("CC") : new MyByte("CB"));
                break;
            case 8:
                opcode = three ? new MyByte("D2") : new MyByte("CC");
                break;
    ...
    public synchronized void run() {
        ...
        if (floating) {
            ...
            for (int i = length - 1; i >= 0; i--) {
                erg[i] = carry ?
                        new MyByte(
                                ope2[i].getContent() + ope1[i].negate().getContent()
                                        + 1) :
                        new MyByte(ope2[i].getContent() + ope1[i].negate()
                                .getContent());
            ...
            machine.getFlags().setZero(erg1 == 0);

            machine.getFlags().setNegative(erg1 < 0);
